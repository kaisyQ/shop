...
export const fetchUsers = createAsyncThunk(
    ...
    async () => {
        try {
            const response = await fetch(`${process.env.BACKEND_PORT || "http://localhost:8000/"}/users`, {
                method: "GET",
                headers: {
...
export const fetchUserById = createAsyncThunk(
    ...
    async (id: string) => {
        try {
            const response = await fetch(`${process.env.BACKEND_PORT || "http://localhost:8000/"}/users/${id}`, {
                method: "GET",
                headers: {
                    "Content-Type": "application/json",
                },
